...
$('document').ready(() => {
  ...
  $('.m-4').empty();
  output +=
    '<h3>Projet RO : Problème de transport</h3><div>Présenté par :</div>';
  output +=
    '<div><label>Nom et prénoms : ' +
    info.name +
    ' ' +
    info.f_name +
    '</label></div>';
  output += '<div><label>Matricule : ' + info.matricule + '</label></div>';
  output += '<div><label>Parcours : ' + info.parcours + '</label></div>';
  $('.m-4').append(output);
});

var info = {
  name: 'ANDRIANIRINAHARIVELO',
  f_name: 'AINA SANDRATRA',
  matricule: '824 H-F',
  parcours: 'M1 IG',
};
